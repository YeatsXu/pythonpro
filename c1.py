...
def demo_string():
    ...
    print 6, len(strc)  #长度
    print 7, '-'.join(['a', 'b', 'c'])  #连起来
    print 8, strc.split(' ')    #空格分开
    print 9, strc.find('ello')  #是否找到字符串
...
def demo_buildinfunction():
    print 1, max(2, 1), min(5, 3)   #最大值、最小值
    print 2, len('xxx'), len([1, 2, 3])     #字符串长度、数组长度
    print 3, abs(-2)  # fabs,Math.fabs  #绝对值
    print 4, range(1, 10, 3)    #主要用在for循环里面
    print 5, dir(list)  # list是类型
    x = 2
    print 6, eval('x + 3')
    print 7, chr(65), ord('a')  #ascii
    print 8, divmod(11, 3)
...
def demo_list():        # 最常用
    ...
    listb = ['a', 1, 'c', 1.1]
    print 2, listb
    lista.extend(listb)     #连起来
    print 3, lista
    print 4, len(lista)
    print 5, 'a' in listb   #‘a’是否在listb
    lista = lista + listb   #   操作符重载
    print 6, lista
    listb.insert(0, 'www')      #位置0前面加www
    print 7, listb
    listb.pop(1)    # listb弹出一个
    print 8, listb
    listb.reverse() #listb逆序
    print 9, listb
...
def demo_dict():    #字典 hashtable   hashmap key->value
